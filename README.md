...
选择原创合成参考声线后，再运行 `--mode generate`。真实人物声线需要用户明确请求并授权；默认使用通用或新设计的合成声线。
...
- 生成画面不得伪装成真实档案。
- 采访片段要核对原视频、原音、转写、人物与时间码。
- 不凭口型判断说了什么。
- 不在未经授权的情况下模仿可识别真人声线。
- 下载、使用和发布素材时，由项目方确认适用的版权、许可、合理使用与平台规则。
- Skill 不会把作者宣传植入用户影片、字幕或文章；只在成功交付后的 Agent 对话回复中显示一次署名。
